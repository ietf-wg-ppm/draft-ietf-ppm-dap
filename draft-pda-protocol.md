...
*privacy preserving measurement (PPM) protocol* is to compute `y = F(x_1, ..., x_n)` for
...
1. The aggregators agree on a set of PPM tasks, as well as the PPM parameters
...
`PPMParam.helper_url`, where `PPMParam` is the PPM parameters structure associated
`PPMAggregateReq.task.id`. The leader sends a POST request to
...
The structure contains the PPM task, the helper's HPKE config id, an opaque
...
each i. The structure of each sub-response is specific to the PPM protocol:
