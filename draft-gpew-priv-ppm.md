...
multi-party computation, specified by the chosen VDAF, to ensure that consuming
the report will not corrupt the aggregate result. The input shares
generated by the client include information used by the aggregators during
aggregation. For example, `prio3` includes a distributed zero-knowledge proof of
the input's validity [BBCGGI19] which the aggregators can jointly verify
and reject the report if it cannot be verified. However, they do not
...
/* A nonce used to uniquely identify a report in the context of a PPM task */
struct {
  Time time;
  uint64 rand;
} Nonce;
...
* `nonce` is a nonce generated by the client and used by the aggregators to
  uniquely identify the report in the context of a PPM task. The nonce includes
  the time at which the report was generated and a random 64-bit integer. This
  field is used by the aggregators to ensure the report appears in at most one
  batch. (See {{anti-replay}}.)
...
the sub-request in the same position in `AggregateReq`. Each sub-response
...
shares that fall in the batch interval into an output share. The response
...
report `R1` if either `R2.time > R1.time` or `R2.time == R1.time` and `R2.rand
> R1.rand`. If `R2.time < R1.time`, or `R2.time == R1.time` but `R2.rand <=
R1.rand`, then we say that `R2` does not follow `R1`.
...
nonce into the AAD for HPKE encryption. Note that this strategy may result in
dropping reports that happen to have the same timestamp and nonce value.
