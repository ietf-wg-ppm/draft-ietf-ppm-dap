...
batch interval for which it has received at least one output-share request (see
{{output-share-request}}), the helper MUST send an error messsage in response
rather than its next VDAF message. Note that this means leaders cannot
interleave a sequence of aggregate and output-share requests for a single batch.
...
the input share is only recovered if the aggregator is given the correct nonce.
(See {{upload-request}}.)
