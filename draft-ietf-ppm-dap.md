...
* `task_end`: The time up to which Clients are expected to upload to this task.
  The task is considered completed after this time. Aggregators MAY reject
  reports that have timestamps later than `task_end`, and MAY reject the task
  if the task lifetime is too long given by `task_end - task_start`.
...
`task_start`, or is past the task's `task_end`. When it does so, it SHOULD also
abort the upload interaction and alert the Client with error `reportRejected`.
Client MAY choose to opt out of the task if its own clock is earlier than
`task_start`, or has passed `task_end`.
...
1. Check if the report's timestamp has passed the task's `task_end` time.
...
current time has not passed this task's `task_end`. Aggregator MAY delete the
task and all data pertaining to this task after `task_end`. Implementors SHOULD
provide for some leeway so the Collector can collect the batch after some delay.
...
| `0x07` | `task_ended`             | {{aggregation-helper-init}} of RFX XXXX |
