...
triggered by receiving a message from the Aggregatro's peer. Eventually this
...
will be processed for it. Each state is said to have one of four possible
"types" (see {{Section 5.7 of !VDAF}}: `Continued`, `FinishedWithOutbound`,
`Finished` and `Rejected`. The first two types include an outbound message to
be processed by the peer.
...
First, the Leader MUST ensure each report in the candidate set can be
committed. That is: the Leader MUST ensure it has not committed a report with
the same ID and task; and the Leader MUST ensure the batch bucket to which the
report belongs has not been collected. See {{batch-buckets}} for details. If a
report is not cannot be committed, then the Leader rejects it and removes it
...
determines the initial per-report state, denoted `state`. If `state` is of type
`Rejected` ({{Section 5.7 of !VDAF}}, then the report is rejected and removed
from the candidate set, and no message is sent to the Helper for this report.
...
`state.outbound`. The Leader uses it to construct a `PrepareInit` message:
...
1. If the inbound prep response has type "continue", then the Leader computes
   ...
   * `inbound` is the payload of the `PrepareResp`
   ...
   Else if the new  `state` has type `Finished`, then preparation is complete
   and the state includes an output share, denoted `state.out_share`. The
   Leader commits to `state.out_share` as described in {{batch-buckets}}.
...
variant {
  ...
  opaque payload<0..2^32-1> = state.outbound;
} PrepareResp;
~~~
...
1. If `state` has type `Continued` and the inbound `PrepareResp` has
   ...
   where `task_id` is the task ID and `inbound` is the payload of the inbound `PrepareResp`.
   It then processes the next state transition:
...
1. Otherwise the inbound message is incompatible with the Helper's current
   state, in which case the Leader MUST abort the aggregation job.
...
where `task_id` is the task ID and `inbound` is the payload of the inbound
`PrepareContinue`. If `state` has type `Rejected`, then the Helper's response
is
