...
The set of candidate reports MUST produce a set of report ID commitments which
does not conflict with the report ID commitments for any other aggregation job
associated with the task (see {{report-id-commitments}}).
...
1. Check if the report has been previously aggregated. If so, the input share
   ...
   considered previously aggregated if it conflicts with a report ID commitment
   for a completed aggregation job (see {{report-id-commitments}}).
...
#### Report ID commitments {#report-id-commitments}

Replay protection is implemented using "report ID commitments". A report ID
commitment is an interval of report IDs (using lexicographic ordering),
associated with a given time window (that is, an interval of time whose duration
is equal to the task's `time_precision` and whose start has a UNIX timestamp
which is a multiple of the task's `time_precision`).

A set of reports for a task determines a set of report ID commitments, producing
one commitment per time window represented in the set of reports. The report ID
commitments associated with a given set of reports are determined by the
following procedure:

1. Partition the set of reports into groups based on the time window their
   client timestamp falls into; this might be achieved by partitioning the
   reports on a key generated by truncating each report's timestamp down to the
   nearest multiple of the task's `time_precision`.
2. For each partitioned group of reports, generate an interval from the smallest
   report ID in the group to the largest report ID in the group, inclusive. The
   report ID commitment for this partition is this interval, associated with the
   time window that generated this partition.

The set of report ID commitments associated with a given aggregation job is the
set of report ID commitments associated with the reports included in the
aggregation job.

Two report ID commitments for a given task are considered to conflict if they
are associated with the same time window, and their report ID intervals
intersect. An individual report is considered to conflict with a report ID
commitment if the report's timestamp falls into the time window associated with
the report ID commitment, and the report's ID is contained in the report ID
commitment's interval.
...
configuration; specifically, the batch boundaries must align with the boundaries
of some sequence of time windows. For the `batch_interval` included with the
...
attacks, it is necessary for each Aggregator to retain a set of report ID
commitments tracking reports that have been aggregated for the task so far.
Depending on the task lifetime and report upload rate, this can result in high
storage costs. To alleviate this burden, DAP allows Aggregators to drop this
state as needed, so long as reports are dropped properly as described in
