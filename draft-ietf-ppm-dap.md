...
{{batch-buckets}}. These values are stored until the batch is collected as
described in {{collect-flow}}.

The aggregation interaction provides protection against including reports in
more than one batch ({{replay-protection}}) and against adding reports to
...
job initialization is ready, in which case, it responds with the
`AggregationJobResp`, or the job fails, in which case the Helper MUST abort with
...
described in {{batch-buckets}}. If commitment fails with error `commit_error`,
then instead of the `continue` response above, the Helper responds with

~~~ tls-presentation
variant {
  ReportID report_id;
  PrepareRespstate prepare_resp_state = reject;
  ReportError = commit_error;
} PrepareResp;
~~~
...
job continuation is ready, in which case, it responds with the
`AggregationJobResp`, or the job fails, in which case it MUST abort with the
...
in {{batch-buckets}}. If commitment fails with some some error `commit_error`,
...
commit more reports to the batch (see {{batch-buckets}}).
...
commit any more output shares to the batch and it is an error for the Leader to
issue any more aggregation jobs for additional reports that satisfy the query.
These reports will be rejected by the Helper as described in {{batch-buckets}}.
...
In such settings, deployments should consider whether the an Aggregator
