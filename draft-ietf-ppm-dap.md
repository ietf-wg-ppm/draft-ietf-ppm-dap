...
- Bump version tag from "dap-16" to "dap-18". (\*)

- Bump draft-irtf-cfrg-vdaf to -19 ({{!VDAF}}).
...
({{Section 5.2 of !VDAF}}). The report generated by the Client includes
...
by the Aggregators. Each aggregation job verifies `k` reports, outputting `k`
...
Each aggregation job verifies `k` reports (each consisting of a public share,
...
discussed in {{eager-aggregation}}) and is used to verify all the reports in
...
~~~
{:
  #object-lifecycle-report-verify
  title="Detail of verification of an individual report"
}
...
A `Url` is the encoding of a URL ({{!RFC3986}}) as ASCII bytes. The encoding of
any particular URL into ASCII is agreed upon by participants out of band from
the protocol. Protocol participants MUST NOT re-encode or otherwise alter a
`Url` when including it in a network message or cryptographic digest.
...
`verify_init` methods (see {{!VDAF, Section 5}}). Additionally, DAP includes
...
parameter disagreement; see {{task-configuration}} and {{task-binding}}.
Encodings for the VDAFs defined in {{!VDAF}} are provided in
...
* `min_batch_size` is the smallest number of reports a batch is allowed to
  include.
* `batch_mode` is the codepoint from the Batch Modes Registry
  ({{batch-mode-reg}}) corresponding to the DAP batch mode the task uses; see
  {{batch-modes-overview}}.
...
An aggregation job runs the VDAF verification process described in {{!VDAF,
Section 5.2}} for each report in the job. Verification has two purposes:
...
1. For each valid report share, initialize VDAF verification (see {{Section 5.2
   of !VDAF}}).
...
`state.outbound`. The Leader uses it to construct a `VerifyInit` structure for
that report.
...
`AggregationJobInitReq` message containing the `VerifyInit` structures
...
* `extensions`: A list of aggregation job extensions ({{agg-job-extensions}})
  providing additional parameters for the aggregation job.
...
1. If the inbound verification response has type "continue", then the Leader
   computes
...
message from the Leader. For each `VerifyInit` in this message, the Helper
attempts to initialize VDAF verification (see {{Section 5.1 of !VDAF}}) just as
...
* Whether the report IDs in `AggregationJobInitReq.verify_inits` are all
...
`VerifyResp.report_id` is always set to the ID of the report that the Helper is
...
sets `VerifyResp.verify_resp_type` to `reject` and `VerifyResp.report_error`
...
`Rejected`, then the Helper sets `VerifyResp.verify_resp_type` to `reject` and
`VerifyResp.report_error` to `vdaf_verify_error`.
...
not reachable at this point. The Helper sets `VerifyResp.verify_resp_type` to
`continue` and `VerifyResp.payload` to `state.outbound`.
...
bucket was collected), then the Helper sets `VerifyResp.verify_resp_type` to
`reject` and `VerifyResp.report_error` to `commit_error`.

Once the Helper has constructed a `VerifyResp` for each report, the aggregation
...
`verify_resps` is the outbound `VerifyResp` messages for each report computed
...
state object `state` for each report in the candidate set. Either all states
...
wants the Helper to advance to. The `verify_continues` field is the sequence of
verification continuation messages constructed in the previous step. Here
`verify_continues_length` is the length of the HTTP message content
({{!RFC9110, Section 6.4}}), minus the length in octets of `step`. The
`VerifyContinue` elements MUST be in the same order as the previous request to
...
The response's `verify_resps` MUST include exactly the same report IDs in the
...
1. If `state` has type `Continued` and the inbound `VerifyResp` has
   ...
   `VerifyResp`, and `state` is the report's verification state carried over
...
1. Else if `state` has type `FinishedWithOutbound` and the inbound
   `VerifyResp` has type "finish", then verification is complete and the
   Leader commits to `state.out_share` as described in
   {{batch-buckets}}.

1. Else if the inbound `VerifyResp` has type "reject", then the Leader rejects
...
previous request, then the Helper MAY fail the job with error `invalidMessage`.
...
where `task_id` is the task ID, `inbound` is the payload of the inbound
`VerifyContinue`, and `state` is the report's verification state carried over
from the previous step. If the new `state` has type `Rejected`, then the Helper
sets `VerifyResp.verify_resp_type` to `reject` and `VerifyResp.report_error`
to `vdaf_verify_error`.
...
`VerifyResp.verify_resp_type` to `reject` and `VerifyResp.report_error` to
`commit_error`.
...
`VerifyResp.verify_resp_type` to `continue` and `VerifyResp.payload` to
`state.outbound`.

Otherwise, if `state` has type `Finished`, then the Helper sets
`VerifyResp.verify_resp_type` to `finish`.

Once the Helper has computed a `VerifyResp` for every report, the aggregation
job response is ready. It is represented by an `AggregationJobResp` message
(see {{aggregation-helper-init}}) with each verification step. The order of the
verification steps MUST match the Leader's `AggregationJobContinueReq`.
...
implementations SHOULD checkpoint the most recent step's verification state and
...
{{agg-job-extensions}}). A batch mode that defines such an extension should
specify how it is used to assign reports to batch buckets. See the
leader-selected mode ({{leader-selected-batch-mode}}) for an example.
...
### Task Configuration Agreement and Consistency {#task-binding}

{{Section 9.9 of !VDAF}} warns of cross protocol attacks, in which a message
recipient can be manipulated into handling a message intended for use in one
protocol as a message from another (i.e., evaluating a `Prio3Count` input share
as though it was a `Prio3Sum` input share).

VDAF also calls out the risk of parameter disagreement, where both parties can
...
too small for the aggregates to protect client privacy.

To rule out these attacks, DAP sets the goal of ensuring that protocol
...
upload reports and when Aggregators deliver aggregate shares.

Besides ensuring that no participant has been tricked into using the wrong
...
called "DAP Batch Mode Identifiers" ({{batch-modes}}). This registry contains
...
### Collection Job Extensions Registry
...
### Report Errors Registry {#report-error-reg}

A new registry will be (RFC EDITOR: change "will be" to "has been") created
called "DAP Report Error Identifiers".
...
* When a document defines a new batch mode, it MUST include a section titled
  "DAP Batch Mode Considerations" specifying the following:
    ...
    * Aggregation job extensions, if any, needed for batch mode operation
...
* When a document defines a new VDAF ({{!VDAF, Section 5}}), it MUST specify an
  ...
  attacks described in {{task-binding}}.
...
`AggregateShareAad` ({{aggregate-share-encrypt}}) structures to prevent attacks
described in {{task-binding}}.
