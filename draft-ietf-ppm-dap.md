...
computation, each aggregator is in posession of either an "output share" that is
...
where `inbound` is the previous VDAF preapre message sent by the leader and
...
* If this is the last round of VDAF preparation phase, then `out` is the
  ...
  future collection.
* Otherwise, the helper interpets `out` as the tuple `(new_state, prep_msg)`,
